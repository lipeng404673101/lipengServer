...
struct CardValue
{
	...
	{
		ReadMapData(obj, NAME_TO_STR(kColor), kColor);
		ReadMapData(obj, NAME_TO_STR(kNumber), kNumber);
		return true;
	}

	bool Write(msgpack::packer<msgpack::sbuffer>& pack)
	{
		WriteKeyValue(pack, NAME_TO_STR(kColor), kColor);
		WriteKeyValue(pack, NAME_TO_STR(kNumber), kNumber);
		return true;
	}
};
...
struct VideoOper
{
	...
	Lstring		ToString()
	{
		...
		{
			ss << m_cards[i].kColor << "|" << m_cards[i].kNumber;
			if (i < m_cards.size() - 1)
			{
				ss << ",";
			}
	...
	void	FromString(Lstring str)
	{
		...
		if (des.size() >= 2)
		{
			...
			for (size_t i = 2; i < des.size(); i ++)
			{
				...
				{
					CardValue card;
					card.kColor = atoi(splids[0].c_str());
					card.kNumber = atoi(splids[1].c_str());
