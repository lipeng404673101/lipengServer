...
struct MHOneDrawActivityHelper
{
	...
	//MHOneDrawActivityHelper() { base_url = "http://test.zjhapi.haoyunlaiyule1.com/push/agency_task?"; }
	MHOneDrawActivityHelper() { base_url = "http://cxapi.tigerb2c.com/push/agency_task?"; }
	...
	inline void Invoke(int roomId, int playType, int userIds[], int score[], int userCount, int rount, Lint startTime)
	{
		...
		LLOG_ERROR("OneDraw target_url=[%s]", target_url.c_str());
...
struct MHPhoneVcodeHelper
{
	...
	MHPhoneVcodeHelper()
	{ 
		message_url = "http://cxapi.tigerb2c.com/server/user_login?";
		voice_url = " http://cxapi.tigerb2c.com/server/user_login_voice?";
	}

	inline void Invoke(Lint type,Lstring phone)
	{
		std::stringstream ss;
		if (type == 1)
		{
		...
		LLOG_ERROR("MHPhoneVcodeHelper URL[%s]",target_url.c_str());
...
struct MHBigWinAndSpecPaiXing
{
	Lstring base_url;
	Lstring target_url;

	//MHBigWinAndSpecPaiXing() { base_url = "http://test.uapi.haoyunlaiyule1.com/club/club_log_task?"; }
	//MHBigWinAndSpecPaiXing() { base_url = "http://wfy.uapi.zhangliediaozhatian.com/push/club_log_task?"; 
	MHBigWinAndSpecPaiXing() { base_url = "http://cxapi.tigerb2c.com/push/club_log_task?"; }

	inline void Invoke(int clubId, int deskId, int userId, std::string & userNike, int state, int paiXingType, int userCount)
	{
		std::stringstream ss;
		ss << base_url;
		ss << "club_no=" << clubId;
		ss << "&user_id=" << userId;
		ss << "&room_number=" << deskId;
		ss << "&play_id=" << state;
		ss << "&type=" << paiXingType;
		ss << "&user_count=" << userCount;
		ss << "&nickname=" << userNike;

		target_url = ss.str();


		LLOG_ERROR("MHBigWinAndSpecPaiXing  target_url=[%s]", target_url.c_str());
