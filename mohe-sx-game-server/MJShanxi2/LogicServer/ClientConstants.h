...
游戏类型编号决定房间人数，比如抠点和二人抠点使用不同的编号
但到了 ShanXiGameHandler 这一层，会把二人抠点转成抠点处理，即在这一层才将游戏类型编号和人数设定分开处理
...
/*
平鲁麻将是流行在山西朔州平鲁一带的麻将玩法
硬三嘴麻将是流行在山西临汾一带的麻将玩法
洪洞王牌麻将是流行在山西临汾一带的麻将玩法
*/

/*
洪洞玩牌创建房间界面修改
1.  小选项中缺两门名称 改为 色牌
2.  小选项顺序： 免碰（默认不选），色牌（默认不选），暗杠可见（默认选），杠不随胡（默认不选）
*/
...
enum  PLAYTYPE
{
  ...
  //// by wyz 20170906 PT_ZhuangJiaYiFen 作以下说明，庄加一分的意思是在设定的庄家风险分基础上加一分，且庄家风险分不为0：FEAT_DealerRiskScore+1。
	...
	PT_QueYiMen = 53,                   //忻州扣点岢岚县打法缺一门选项（**Ren 2017-11-28）
	// 2017-11-25
	MH_PT_GuaSanJiao_High_Score = 54,     // 拐三角高分玩法，平胡5分，坎胡10分, 和大唐分值一致
	MH_PT_HongZhongHaoZi = 55,    //红中癞子小选项 (**Ren 2017-12-6)
	MH_PT_HongTongWangPai_ShuangWang = 56,  //洪洞王牌：添加双王大将报牌小选项（报听打出去的牌只能是中发白）（**Ren 2017-12-11）
	MH_PT_Linfen_DuanGuo = 57, // 临汾硬三嘴，一门牌加选项端锅(封顶50分),2018-1-23
	MH_PT_NianZhongZiErZhongZi = 58,  //撵中子添加双中子报听选项（**Ren 2018-01-22）
	// 2018-1-8, 陵川麻将未上听点炮包胡选项,去掉改选项，默认支持
...
struct GameOptionSet {
	...
	bool b_daiFeng;//带风
	bool b_zimoHu;//只可自摸和
	bool b_HaoZi;  //捉耗子,扣点专用（**Ren 2017-12-13）
...
特点:
1. 推到胡报听
2. 靠八张 主牌够八张报听（主牌够八张，包括碰杠牌，主牌至少2副牌加一对将)，7小对至少4对
3. 积分按照主牌的张数算分（明杠加1张，暗杠另加2张)
4. 未上听点炮包赔，点炮多出2分
5. 杠不算分，胡牌玩家的杠已算张数，没胡牌的杠不算分
6. 包庄：荒庄庄家每人输2分，可继续连庄------去掉
7. 庄家输赢分数翻倍
8. 特殊牌型: 7对，豪华7对， 十三幺可以成牌，但不加分， 清一色不加分， 十三幺成牌加14分
*/
////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////
/*
...
2. 可碰，杠，不可吃，自摸胡或接炮胡
3. 听后点炮不包胡包杠，各自算分
3. 没听牌点炮包胡包杠，桌子上所有人的胡分和杠分
4. 后包：荒庄庄家每人输2分，继续连庄
7. 庄分：庄家胡牌分数翻倍，闲家胡牌庄家扣分翻倍
8. 特殊牌型: 7对，豪华7对， 十三幺胡牌积分翻倍
9. 杠后开花，算自摸
10. 抢杠胡， 算接炮平胡
11. 十三幺，不拾不抢
算分：庄家接炮平胡6分，自摸12分
