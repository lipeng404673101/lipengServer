...
bool Work::Init()
{
	...
	//double d1 = 0.0f;
	//double d2 = 0.0f;
	//get_gps_pair_values(s, d1, d2);
	//LLOG_DEBUG("**************value %f %f", d1, d2);
	//
	//Lstring s1("abc,efg,hij|lmn|||");
	//Lstring s2 = string_replace(s1, Lstring(","), Lstring(" "));
	//LLOG_DEBUG("**************s2: %s", s2.c_str());
	//s2 = string_replace(s2, Lstring("|"), Lstring(""));
	//LLOG_DEBUG("**************s2: %s", s2.c_str());
	...
	m_logFileName = Lstring("LogicServer_") + Lstring(buf);
	...
	if (!gPaiXingMgr.Init())
	{
		LLOG_ERROR("gPaiXingMgr.Init() error");
		return false;
	}
	//初始化机器人
...
{
	LTime msgTime;
	srand(msgTime.Secs() + gConfig.GetServerID() * 77);
	int iMsgCount = 0;
	int iElapseTime = 0;
...
{
	LLOG_DEBUG("Work::HanderMsg  msg->m_msgId=[%d]",msg->m_msgId);
	//玩家请求登录
	switch(msg->m_msgId)
	{
...
void Work::HanderHearBeatReply(LMsgHeartBeatReplyMsg* msg)
{
	...
	//if (2==fromwho)
	//{
	//	LLOG_DEBUG("Logwyz ...  reply  heartbeat   from   manager");
	//	if (m_needSynchroData2Manager)
	//	{
	//		LLOG_DEBUG("Logwyz ... m_needSynchroData2Manager=true ");
	//		_SynchroData2Manager();
	//	}
	//}
}
...
void Work::handerLMGEnterClubDesk(LMsgLMG2LEnterClubDesk*msg)
{
	...
	{
		//不做处理，这个时候不应该在这里
		LLOG_DEBUG("Work::handerLMGEnterClubDesk   pJoinUser && pJoinUser->GetDesk()    not in here");
		LMsgS2CAddDeskRet ret;
		ret.m_deskId=msg->m_deskId;
		ret.m_errorCode=5;
		pJoinUser->Send(ret);
	...
	else
	{
		...
		if (pJoinUser)
		{
			...
			else
			{
				...
				if (nError == 2)
				{
					LLOG_ERROR("handerLMGEnterClubDesk error ,RecycleDeskId desk clubId[%d],playId[%d],clubDeskId[%d],deskId[%d]", msg->m_clubId, msg->m_playTypeId, msg->m_clubDeskId, msg->m_deskId);
					RecycleDeskId(msg->m_deskId, msg->m_clubId, msg->m_playTypeId, msg->m_clubDeskId);
				}
			}
		}
