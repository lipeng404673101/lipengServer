...
inline tile_t ToTile(CardValue const &card) { return (tile_t)(card.kColor * 10 + card.kNumber); }
...
inline CardValue ToCardValue(const Card& card) {
	CardValue v;
	v.kColor = card.m_color;
	v.kNumber = card.m_number;
	return v;
}
inline CardValue ToCardValue(const Card* card) {
...
inline Card ToCard(const CardValue& v) {
	Card card;
	card.m_color = v.kColor;
	card.m_number = v.kNumber;
	return card;
}
...
inline bool operator ==(const Card* lhs, const CardValue& rhs) { return lhs->m_color == rhs.kColor && lhs->m_number == rhs.kNumber; }
inline bool operator ==(const CardValue& lhs, const Card* rhs) { return lhs.kColor == rhs->m_color && lhs.kNumber == rhs->m_number; }
