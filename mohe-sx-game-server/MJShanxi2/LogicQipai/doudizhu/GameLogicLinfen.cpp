...
#define MAX_COUNT_LINFEN				20									//最大数目
...
static struct tagAnalyseResult
{
...
//扑克数据
const BYTE	CGameLogicLinfen::m_cbCardData[FULL_COUNT_LINFEN]=
{
...
const BYTE	CGameLogicLinfen::m_cbCardData[FULL_COUNT_LINFEN]=
{
	0x01, 0x23, 0x39, 0x33, 0x03, 0x06, 0x16, 0x08, 0x26, 0x0A, 0x0B, 0x0C, 0x0D,	//方块 A - K
	0x11, 0x13, 0x14, 0x15, 0x07, 0x17, 0x18, 0x38, 0x1A, 0x1B, 0x1C, 0x1D,	//梅花 A - K
	0x21, 0x22, 0x02, 0x37, 0x25, 0x27, 0x09, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D,	//红桃 A - K
	0x31, 0x04, 0x34, 0x35, 0x36, 0x24, 0x19, 0x05, 0x3A, 0x3B, 0x3C, 0x3D,	//黑桃 A - K
	0x4E, 0x4F
};
#endif
*/
...
//构造函数
CGameLogicLinfen::CGameLogicLinfen()
{
}
...
//获取类型
BYTE CGameLogicLinfen::GetCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
	//简单牌型
	switch (cbCardCount)
	{
	...
	case 2:	//对牌火箭
		{
			...
			if (GetCardLogicValue(cbCardData[0])==GetCardLogicValue(cbCardData[1])
				&&GetCardLogicValue(cbCardData[0])==3
				)  //同色的两个3为炸
				return CT_BOMB_CARD;
			if (   (GetCardLogicValue(cbCardData[0])==GetCardLogicValue(cbCardData[1]) )
				&&    (GetCardValue(cbCardData[0])==2)
				)  //两张2为炸
				return CT_BOMB_CARD;

			if (GetCardLogicValue(cbCardData[0])==GetCardLogicValue(cbCardData[1])) return CT_DOUBLE;

			return (GetCardLogicValue(cbCardData[0])==GetCardLogicValue(cbCardData[1]))?CT_DOUBLE:CT_ERROR;
		}
	}
...
//排列扑克
VOID CGameLogicLinfen::SortCardList(BYTE cbCardData[], BYTE cbCardCount, BYTE cbSortType)
{
	//数目过虑
	...
	//转换数值
	BYTE cbSortValue[MAX_COUNT_LINFEN];
...
//混乱扑克
VOID CGameLogicLinfen::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	//混乱准备
...
//删除扑克
bool CGameLogicLinfen::RemoveCardList(const BYTE cbRemoveCard[], BYTE cbRemoveCount, BYTE cbCardData[], BYTE cbCardCount)
{
	//检验数据
	ASSERT(cbRemoveCount<=cbCardCount);

	//定义变量
	BYTE cbDeleteCount=0,cbTempCardData[MAX_COUNT_LINFEN];
...
//删除扑克
bool CGameLogicLinfen::RemoveCard(const BYTE cbRemoveCard[], BYTE cbRemoveCount, BYTE cbCardData[], BYTE cbCardCount)
{
	//检验数据
	ASSERT(cbRemoveCount<=cbCardCount);
...
//排列扑克
VOID CGameLogicLinfen::SortOutCardList(BYTE cbCardData[], BYTE cbCardCount)
{
	//获取牌型
...
//逻辑数值
BYTE CGameLogicLinfen::GetCardLogicValue(BYTE cbCardData)
{
	//扑克属性
...
//对比扑克
bool CGameLogicLinfen::CompareCard(const BYTE cbFirstCard[], const BYTE cbNextCard[], BYTE cbFirstCount, BYTE cbNextCount)
{
	//获取类型
	...
	case CT_BOMB_CARD:
		{
			...
			if (cbNextCount>cbFirstCount)return true;

			//BYTE cbNextValue=GetCardValue(cbNextCard[0]);
			//BYTE cbFirstValue=GetCardValue(cbFirstCard[0]);

			//对比扑克  处理两个2，两个3为炸的情况
			if (cbFirstLogicValue==3&&cbNextLogicValue==3)return false;
			if (cbFirstLogicValue==2&&cbNextLogicValue!=3)return true;
			if (cbFirstLogicValue!=3&&cbNextLogicValue==2)return false;
...
//分析扑克
VOID CGameLogicLinfen::AnalysebCardData(const BYTE cbCardData[], BYTE cbCardCount, tagAnalyseResult & AnalyseResult)
{
	//设置结果
...
bool  CGameLogicLinfen::HaveSpecCard(const BYTE a_cbCardData[], BYTE cbCardCount)
{
	tagAnalyseResult  tempAR;
	memset(&tempAR, 0x00, sizeof(tempAR));

	BYTE cbCardData[MAX_COUNT_LINFEN];
	memset(cbCardData, 0x00, sizeof(cbCardData));
	memcpy(cbCardData, a_cbCardData, cbCardCount);

	SortCardList(cbCardData, cbCardCount, ST_ORDER);

	AnalysebCardData(cbCardData, cbCardCount, tempAR);

	bool IsJoker = false, litterJoker = false;
	for (int i = 0; i < tempAR.cbBlockCount[0]; i++)
	{
		if (tempAR.cbCardData[0][i] == 0x4E)litterJoker = true;
		if (tempAR.cbCardData[0][i] == 0x4F)IsJoker = true;

		if (litterJoker == true && IsJoker == true)return true;
	}

	for (int i = 0; i < tempAR.cbBlockCount[3]; i++)
	{
		if (GetCardLogicValue(tempAR.cbCardData[3][0]) == 3)
			return true;
	}
...
//构造扑克
BYTE CGameLogicLinfen::MakeCardData(BYTE cbValueIndex, BYTE cbColorIndex)
{
return (cbColorIndex<<4)|(cbValueIndex+1);
}

//分析分布
VOID CGameLogicLinfen::AnalysebDistributing(const BYTE cbCardData[], BYTE cbCardCount, tagDistributing & Distributing)
{
	//设置变量
...
//出牌搜索
BYTE CGameLogicLinfen::SearchOutCard( const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, 
...
//同牌搜索
BYTE CGameLogicLinfen::SearchSameCard( const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbReferCard, BYTE cbSameCardCount,
								tagSearchCardResult *pSearchCardResult )
{
	//设置结果
...
//带牌类型搜索(三带一，四带一等)
BYTE CGameLogicLinfen::SearchTakeCardType( const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbReferCard, BYTE cbSameCount, BYTE cbTakeCardCount, 
									tagSearchCardResult *pSearchCardResult )
{
	//设置结果
	if( pSearchCardResult )
...
//连牌搜索
BYTE CGameLogicLinfen::SearchLineCardType( const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbReferCard, BYTE cbBlockCount, BYTE cbLineCount,
									tagSearchCardResult *pSearchCardResult )
{
	//设置结果
	if( pSearchCardResult )
...
//搜索飞机
BYTE CGameLogicLinfen::SearchThreeTwoLine( const BYTE cbHandCardData[], BYTE cbHandCardCount, tagSearchCardResult *pSearchCardResult )
{
	//设置结果
	if( pSearchCardResult )
		ZeroMemory(pSearchCardResult,sizeof(tagSearchCardResult));
