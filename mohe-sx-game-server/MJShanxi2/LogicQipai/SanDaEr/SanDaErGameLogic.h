#ifndef _SAN_DA_ER_GAME_LOGIC_H_
#define _SAN_DA_ER_GAME_LOGIC_H_
...
protected:
	...
	/* 一副牌的牌值 */
	static const int m_allCardsData[SANDAER_CELL_PACK];
...
public:
	...
	/* 获取牌的逻辑值 -- 打副使用 */
	int GetCardLogicValue_playFu(int cardData);

	/* 获取牌的逻辑花色 */
	int GetCardLogicColor(int cardData);

	/* 获取牌的逻辑花色 -- 打副使用 */
	int GetCardLogicColor_playFu(int cardData);
...
public:
	...
	/* 对比扑克逻辑值 -- 打副使用*/
	bool CompareCardData_playFu(int firstCard, int secondCard);

	/* 找出集合中最大的逻辑牌值对应的位置 */
	int GetMaxCardIndexByLogicValue(const int cardsData[], int cardsCount);

	/* 找出集合中最小的逻辑牌值对应的位置 */
	int GetMinCardIndexByLogicValue(const int cardsData[], int cardsCount);

	/* 找出集合中最大的逻辑牌值 */
	int GetMaxCardByLogicValue(const int cardsData[], int cardsCount);

	/* 找出集合中最小的逻辑牌值 */
	int GetMinCardByLogicValue(const int cardsData[], int cardsCount);
	...
	/* 根据牌花色提取该花色中最大的扑克 */
	int DistillMaxCardByColor(const int cardsData[], int cardsDataCount, int resultCards[], bool isZhuang);

	/* 提取主牌集合,返回集合张数 */
	int DistillMainCards(const int cardsData[], int cardsDataCount, int resultCards[]);

	/* 判断闲家交牌 */
	bool JudgeXianLose(const int allPlayerHandCards[][SANDAER_HAND_CARDS_COUNT_MAX], const int handCardsCount[], int playerCount, int zhuangPos, int firstOutPos);
	
};



#endif // !_SAN_DA_ER_GAME_LOGIC_H_