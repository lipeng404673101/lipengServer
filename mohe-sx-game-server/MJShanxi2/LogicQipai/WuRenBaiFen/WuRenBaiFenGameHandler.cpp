...
public:
	...
	********************************************************/
	...
	/******************************************************
	* 函数名：   WuRenBaiFenIsGanKouDi()
	* 描述：     闲家干抠底
	* 详细：     是否含十：false-不干抠底  true-干抠底
	* 返回：     (bool)允许闲家干抠底
	********************************************************/
	bool  WuRenBaiFenIsGanKouDi() const;

	/******************************************************
	* 函数名：   WuRenBaiFenIsJiaoPai()
	* 描述：     是否允许庄家交牌
...
struct WuRenBaiFenRoundState__c_part
{
	...
	WURENBAIFEN_OVER_STATE             m_wurenbaifen_overstate;               // 新的结束状态

	GAME_PLAY_STATE		m_play_status;										// 牌局状态
	OVER_STATE			m_over_state;										//小结状态

	Lint				m_user_status[WURENBAIFEN_PLAYER_COUNT];				// 用户状态：改位置上是否坐人了（实际有效位位置）
	BOOL				m_tick_flag;										// 定时器开关

	Lint				m_mainColor;										//主牌花色

	Lint				m_max_select_score;									//选择最大的叫牌分数
	Lint				m_select_score[WURENBAIFEN_PLAYER_COUNT];				//玩家叫牌分数

	Lint				m_base_cards_count;									//底牌数量
	Lint				m_base_cards[WURENBAIFEN_BASE_CARDS_COUNT];				//底牌

	Lint				m_hand_cards_count[WURENBAIFEN_PLAYER_COUNT];			//各个玩家手牌数量
	Lint				m_hand_cards[WURENBAIFEN_PLAYER_COUNT][WURENBAIFEN_HAND_CARDS_COUNT_MAX];		//玩家手牌

	Lint				m_out_cards[WURENBAIFEN_PLAYER_COUNT];					//玩家每轮出的牌
	Lint				m_prev_out_cards[WURENBAIFEN_PLAYER_COUNT];				//玩家每轮出的牌
	Lint             m_out_cards_last_turn[WURENBAIFEN_PLAYER_COUNT]; //最后一轮出牌

	Lint				m_scoreCardsCount;									//得分扑克数目
	Lint				m_scoreCards[12 * WURENBAIFEN_PACK_COUNT];				//得分扑克
	Lint             m_scoreCardsLastTurn[WURENBAIFEN_PLAYER_COUNT];     // 最后一手分牌
	Lint             m_scoceCardsCountLastTurn;                                                      // 最后一手分扑克
	Lint				m_leftover_score;									//本局剩余的分

	Lint				m_player_score[WURENBAIFEN_PLAYER_COUNT];				//玩家本局得分

	Lint				m_reset105[WURENBAIFEN_PLAYER_COUNT];					//满105申请结束
	Lint				m_resetTime105;										//时间
	BOOL				m_b105;												//本轮是否申请过105了

	// 交牌控制
	Lint				m_jiaoPai[WURENBAIFEN_PLAYER_COUNT];				//每个人对待交牌的处理结果 -2:初始状态， -1:等待选择， 0:不同意  1:同意 2:已超时
...
struct WuRenBaiFenMatchState__c_part
{
	...
	Lint		m_accum_score[WURENBAIFEN_PLAYER_COUNT];	//存储玩家总得分
	BOOL		m_dismissed;
	...
	BOOL		m_cheatAgainst;							//专属防作弊： 0不启用  1启用
	BOOL		m_isGanKouDi;							//是否允许闲家干抠底：  0：不允许  1：允许
	Lint 		m_nJiaoPaiOption;							//是否允许庄家交牌：    0：不予许  1: 8分交，2:12分交
	bool m_bAnyScoreCanSelectMainColor; // 任意分选主花色 
...
{
	WRBFGameLogic   m_gamelogic;     // 游戏逻辑
...
/*
*  三打二每局所需要的所有字段（结构体）
*  继承 ：WuRenBaiFenRoundState__c_pard, WuRenBaiFenRoundState_cxx_part
*/
...
/*
*  三打二每场所需要的所有字段（结构体）
*  继承：WuRenBaiFenMatchState__c_pard, WuRenBaiFenMatchState_cxx_pard
*/
...
struct WuRenBaiFenVideoSupport : WuRenBaiFenDeskState
{
	...
	//五人百分 oper 操作  0--过  1--出牌   位置   手牌个数   手牌 操作的牌个数  操作的牌  得分
...
struct WuRenBaiFenGameCore : GameHandler, WuRenBaiFenVideoSupport
{
	...
	bool on_event_user_select_score(Lint pos, Lint selectScore)
	{
		...
		if (has_opt_over(GAME_PLAY_SELECT_SCORE) || m_select_score[pos] == 100)
		{
			...
			for (Lint i = 0; i < WURENBAIFEN_PLAYER_COUNT; ++i)
			{
				...
				baseCardN.m_jiaopaiOption = m_nJiaoPaiOption;// 设置交牌标志
	...
	bool on_event_user_out_card(Lint pos, Lint outCard)
	{
		...
		if (m_hand_cards_count[pos] == 1)
		{
			...
			if (m_wTurnWinner != m_zhuangPos && m_wTurnWinner != m_friendZhuangPos)
			{
				...
				m_wurenbaifen_overstate = WURENBAIFEN_OVER_STATE::DIPAI_XIAN_WIN; //闲扣底
			}
			else
			{
				m_wurenbaifen_overstate = WURENBAIFEN_OVER_STATE::DIPAI_ZHUANG_WIN;     //庄扣底
				if (m_scoreCardsCount == 0)
				{
					m_wurenbaifen_overstate = WURENBAIFEN_OVER_STATE::DIPAI_ZHUANG_WIN_XIAN_SCORE0; //光头
				}
			}
		...
		else
		{
			...
			if (m_curPos == WURENBAIFEN_INVALID_POS)
			{
				...
				LLOG_DEBUG("WuRenBaiFenGameHandler::on_event_user_out_card() 一轮结束了2, deskId=[%d],m_thisTurnScore =[%d]",
				...
				//m_wTurnWinner = WURENBAIFEN_INVALID_POS;		//一轮出完了,最大的重置

				LLOG_DEBUG("WuRenBaiFenGameHandler::on_event_user_out_card() 一轮结束了1, deskId=[%d], currPos=[%d], optPos=[%d], outCard=[0x%2x|%2d]",
					m_desk ? m_desk->GetDeskId() : 0, m_curPos, pos, outCard, outCard);
	...
	*/
	...
	void CalcScoreForWuRenBaiFen()
	{
		...
		Lint xian_desk_score = m_gamelogic.GetCardsScore(m_scoreCards, m_scoreCardsCount); //闲家检分
		Lint last_turn_score = m_gamelogic.GetCardsScore(m_scoreCardsLastTurn, m_scoceCardsCountLastTurn); //最后一轮分值
		Lint dipai_score = m_gamelogic.GetCardsScore(m_base_cards, m_base_cards_count); //最后一轮分值
		...
		else if (m_wurenbaifen_overstate == WURENBAIFEN_OVER_STATE::DIPAI_ZHUANG_WIN) //庄保底
		{
			Lint judge_score = max_call_score + xian_desk_score - 100;
			if (judge_score > 0) // 闲家赢
			{
				this_score = -x_part;
			}
			else                            //庄家赢
		...
		else if (m_wurenbaifen_overstate == WURENBAIFEN_OVER_STATE::DIPAI_ZHUANG_WIN_XIAN_SCORE0) // 庄保底光头
		{
			this_score = 3;
			b_first_xian = true;
			for (int i = 0; i < WURENBAIFEN_PLAYER_COUNT; i++)
		...
		else if (m_wurenbaifen_overstate == WURENBAIFEN_OVER_STATE::DIPAI_XIAN_WIN) //闲扣底
		{
			for (int i = 0; i < WURENBAIFEN_PLAYER_COUNT; i++)
			{
				if (m_isGanKouDi) //干扣底玩法
				{
					this_score = 4;
					if (i == m_zhuangPos || i == m_friendZhuangPos) continue;
					m_player_score[i] += this_score;
...
struct WuRenBaiFenGameHandler : WuRenBaiFenGameCore
{
	...
	************************************************************************************************************/
	...
	void OnUserReconnect(User* pUser)
	{
		...
		// 百分游戏不再需要这个响应
		/*
		if (m_reset105[pos] != -2 && m_b105 && pos != WURENBAIFEN_INVALID_POS)	//有105消息  且未选择
		{
